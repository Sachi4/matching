...
- 表示件数を絞るとき（`/screen` の3枚など）は新しい順に切るのではなく、感情ごとの列から
  1件ずつ取って埋め、その順番のまま並べる。単純に新しい順だと、直近にまとめて成立した
  1つの感情だけで埋まってしまう
- `/screen` は横スクロールしてもらえないので、1920px幅に一度に入る3枚だけ出す
...
- 強く寄っている感情があればそのハブの近くに、まんべんなく響く人は中央に集まる
  （寄りの差が小さいと中央に固まって名前が読めないので、配置の重心だけ `weight^3` で尖らせている）
- 誰がどこにいるか分からないと意味がないので、参加者のニックネームは常に表示する（背景色の縁取り付き）
...
- 閾値を超えたペアはオレンジの太い線でつながり、線の上に「共鳴度％・決め手の感情」を出す
  （`/screen` はラベルが多いと読めないので、感情ごとに共鳴度が最も高い1組だけに絞る。
  感情を問わず上位から取ると、特定の感情のペアだけが並んでしまう）
  （`edges.score` / `edges.decisive_tone_id` = `matches` の値。解釈が要る反応名はマップには出さず、カード側だけ）
- 自分が含まれるペアの線は太くして、自分が誰とどこで共鳴したかを追えるようにしている
