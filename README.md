...
4. `/screen` … 会場の大画面用ビュー（星座と共鳴カード）
   - `/screen/mosaic` … 感想が記録されるたびに「印」のタイルが1枚増えるモザイクウォール
...
- フロントは Supabase Realtime（`postgres_changes`）で `matches` / `stimulus_responses` の insert を購読し、
  共鳴フィードとモザイクウォールを即時更新し、共鳴の瞬間はバースト演出を出す
...
- `ResonanceBurst`（`/respond` `/feed` `/screen` `/admin/test`）が `matches` の insert を購読し、
  画面中央に数秒間のバースト演出（広がる光の輪と弾ける粒子）を出す。タップで閉じる
- 中央には2人の「印」が重なり合い、その下に実際に選んだヒント語を並べる。
  生成AIの反応名はそれより小さい添え物として表示する
- 「印」（`src/lib/sigil.ts`）は回答行のIDをseedにしたハッシュから決定的に生成される有機的な図形で、
  色はその回答のトーンのパレット。同じ人・同じ回答ならいつも同じ印になる（モザイクのタイルと共通）
- スマホでは `navigator.vibrate()` で短い振動を出す。会場の大画面（`/screen`）は振動も音もなし
